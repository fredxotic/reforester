...
// services/speciesApi.js - Update the POPULAR_SPECIES_IMAGES
const POPULAR_SPECIES_IMAGES = {
  ...
  'terminalia-brownii': 'https://i.pinimg.com/1200x/be/4a/13/be4a130815376cc8137d2d673bd239a4.jpg', // Fallback tree image
  'commiphora-africana': 'https://images.unsplash.com/photo-1570458436416-b8c9ccbd37f9?w=320&h=320&fit=crop', // Fallback tree image
  'faidherbia-albida': 'https://images.unsplash.com/photo-1570458436416-b8c9ccbd37f9?w=320&h=320&fit=crop', // Fallback tree image
  'balanites-aegyptiaca': 'https://images.unsplash.com/photo-1570458436416-b8c9ccbd37f9?w=320&h=320&fit=crop', // Fallback tree image
  'azadirachta-indica': 'https://images.unsplash.com/photo-1598301257982-0cf01499abb2?w=320&h=320&fit=crop' // Fallback tree image
};
...
async function searchGBIFSpecies(rawQuery, cleanQuery) {
  ...
  // the “q” param does fuzzy/search across all name fields
  params.set('q', rawQuery);
  // Also try to match common names / vernaculars
  params.set('qField', 'VERNACULAR');
  // Only get species
  ...
  const withImages = await Promise.all(
    results.map(async sp => {
      try {
        ...
        console.warn('Image fetch error for', sp.scientificName, e);
      }
      return sp;
    })
  );
...
async function getGBIFImages(speciesKey) {
  try {
    ...
    console.warn('getGBIFImages error', e);
    return [];
  }
}
